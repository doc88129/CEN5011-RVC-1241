...
def showSignInPopup():
    ...
    if st.button('Sign In'):
        ...
        elif db_utils.verifyLogin(conn, username, password):
...
def showSignUpPopup():
    ...
    if st.button('Create Account'):
        uExist = db_utils.check_username(conn, newUsername)
        eExist = db_utils.checkEmail(conn, newEmail)
        print(uExist, eExist)
        if not (newUsername) or not (newPassword) or not (newEmail) or not(verifyPass) or not(gender) or not(age) or not(weight) or not(height1) or not (height2):
            ...
            db_utils.add_user(conn, newUsername, newPassword, newEmail, height, weight, age, gender)

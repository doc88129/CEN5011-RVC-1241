...
def showSignInPopup():
    ...
    if st.button('Sign In'):
        ...
        elif verifyLogin(conn, username, password):
...
def showSignUpPopup():
    ...
    if st.button('Create Account'):
        uExist = check_username(conn, newUsername)
        eExist = checkEmail(conn, newEmail)
        print(uExist, eExist)
        if not (newUsername) or not (newPassword) or not (newEmail) or not(verifyPass) or not(gender) or not(age) or not(weight) or not(height1) or not (height2):
            ...
            add_user(conn, newUsername, newPassword, newEmail, height, weight, age, gender)
